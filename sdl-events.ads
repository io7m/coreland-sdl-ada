...
package SDL.events is
  ...
  function PeepEvents (events: event_ptr_t; numevents: c.int;
    action: event_action_t; mask: event_mask_t) return c.int;
  function peep_events (events: event_ptr_t; numevents: c.int;
    action: event_action_t; mask: event_mask_t) return c.int renames PeepEvents;
  ...
  function EventState (the_type: event_type_t; state: c.int) return uint8;
  function event_state (the_type: event_type_t; state: c.int) return uint8 renames EventState;
  procedure EventState (the_type: event_type_t; state: c.int);
  procedure event_state (the_type: event_type_t; state: c.int) renames EventState;
